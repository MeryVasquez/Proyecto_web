...
<div class="container">
    <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-2">
            ...
            $strsql = "SELECT idproducto, nombre_producto, idcategoria, idmarca, descripcion, precio, cantidad, url_imagen FROM productos where idmarca=?";
            if ($stmt = $mysqli->prepare($strsql)) {
                $stmt->bind_param("i", $idmarca);
                $stmt->execute();
                $stmt->store_result();
                if ($stmt->num_rows > 0) {
                    $stmt->bind_result($idproducto, $nombre_producto, $idcategoria, $idmarca, $descripcion, $precio, $cantidad, $url_imagen);
                    while ($stmt->fetch()) {
                        ?>
